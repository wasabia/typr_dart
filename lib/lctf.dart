...
class Typr_LCTF {
  ...
  static Map<String, dynamic> parse(data, offset, length, font, subt)
  {
    ...
    var offScriptList  = TyprBin.readUshort(data, offset);  offset += 2;
    var offFeatureList = TyprBin.readUshort(data, offset);  offset += 2;
    var offLookupList  = TyprBin.readUshort(data, offset);  offset += 2;
  ...
  static readLookupList(data, offset, subt)
  {
    ...
    {
      var noff = TyprBin.readUshort(data, offset);  offset+=2;
  ...
  static Map<String, dynamic> readLookupTable(data, offset, subt)
  {
    ...
    obj["ltype"] = TyprBin.readUshort(data, offset);  offset+=2;
    obj["flag"]  = TyprBin.readUshort(data, offset);  offset+=2;
  ...
  static readClassDef(data, offset)
  {
    ...
    if(format==1) 
    {
      var startGlyph  = TyprBin.readUshort(data, offset);  offset+=2;
      var glyphCount  = TyprBin.readUshort(data, offset);  offset+=2;
      for(var i=0; i<glyphCount; i++)
      {
        obj.add(startGlyph+i);
        obj.add(startGlyph+i);
        obj.add(TyprBin.readUshort(data, offset));  offset+=2;
      }
    }
    if(format==2)
    {
  ...
  static readFeatureList(data, offset)
  {
    ...
    for(var i=0; i<count; i++)
    {
      var tag = TyprBin.readASCII(data, offset, 4);  offset+=4;
      var noff = TyprBin.readUshort(data, offset);  offset+=2;
  ...
  static Map<String, dynamic> readFeatureTable(data, offset)
  {
    ...
    var featureParams = TyprBin.readUshort(data, offset); offset+=2;
    if (featureParams > 0) {
      feat["featureParams"] = offset0 + featureParams;
    }
    
    var lookupCount = TyprBin.readUshort(data, offset);  offset+=2;
    feat["tab"] = [];
    for(var i=0; i<lookupCount; i++) feat["tab"].add(TyprBin.readUshort(data, offset+2*i));
    return feat;
  }
  ...
  static Map<String, dynamic> readScriptTable(data, offset)
  {
    ...
    var defLangSysOff = TyprBin.readUshort(data, offset);  offset+=2;
    obj["default"] = readLangSysTable(data, offset0 + defLangSysOff);
    
    var langSysCount = TyprBin.readUshort(data, offset);  offset+=2;
    
    for(var i=0; i<langSysCount; i++)
    {
      var tag = TyprBin.readASCII(data, offset, 4);  offset+=4;
      var langSysOff = TyprBin.readUshort(data, offset);  offset+=2;
  ...
  static Map<String, dynamic> readLangSysTable(data, offset)
  {
    ...
    var lookupOrder = TyprBin.readUshort(data, offset);  offset+=2;
    //if(lookupOrder!=0)  throw "lookupOrder not 0";
    obj["reqFeature"] = TyprBin.readUshort(data, offset);  offset+=2;
    //if(obj.reqFeature != 0xffff) throw "reqFeatureIndex != 0xffff";
    
    //console.warn(lookupOrder, obj.reqFeature);
    
    var featureCount = TyprBin.readUshort(data, offset);  offset+=2;
