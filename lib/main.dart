...
class Typr {
  ...
  static parse(Uint8List buff) {
    ...
    if(tag=="ttcf") {
      var offset = 4;
      var majV = TyprBin.readUshort(data, offset);  offset+=2;
      var minV = TyprBin.readUshort(data, offset);  offset+=2;
      var numF = TyprBin.readUint  (data, offset);  offset+=4;
      var fnts = [];
      for(var i=0; i<numF; i++) {
        var foff = TyprBin.readUint  (data, offset);  offset+=4;
  ...
  static _readFont(data, offset) {
    ...
    for(var i=0; i<numTables; i++)
    {
      ...
      var checkSum = TyprBin.readUint(data, offset);  offset += 4;
      var toffset = TyprBin.readUint(data, offset);   offset += 4;
