...
class Typr_CFF {
	...
	static parse(Uint8List buffer, offset, length) {
		...
		for(var i=0; i<ninds.length-1; i++) names.add(TyprBin.readASCII(data, offset+ninds[i], ninds[i+1]-ninds[i]));
		...
		// Top DICT Data
		var topDicts = [];
		for(var i=0; i<tdinds.length-1; i++) topDicts.add( readDict(data, offset+tdinds[i], offset+tdinds[i+1]) );
		...
		// String Data
		var strings = [];
		for(var i=0; i<sinds.length-1; i++) strings.add(TyprBin.readASCII(data, offset+sinds[i], sinds[i+1]-sinds[i]));
		...
		if(topdict["CharStrings"] != null)
		{
			...
			var cstr = [];
			for(var i=0; i<sinds.length-1; i++) cstr.add(TyprBin.readBytes(data, offset+sinds[i], sinds[i+1]-sinds[i]));
			//offset += sinds[sinds.length-1];
		...
		if(topdict["ROS"] != null) {
			...
			if(fmt==3) {
				var rns = TyprBin.readUshort(data, offset);  offset+=2;
				for(var i=0; i<rns+1; i++) {
					topdict["FDSelect"].addAll([ TyprBin.readUshort(data, offset), data[offset+2] ]);  
          offset+=3;
				}
			}
			else throw fmt;
  ...
  static readSubrs(data, offset, obj) {
		...
		obj["Subrs"] = [];
		for(var i=0; i<gsubinds.length-1; i++) obj["Subrs"].add(TyprBin.readBytes(data, offset+gsubinds[i], gsubinds[i+1]-gsubinds[i]));
  ...
  static var tableSE = [
	...
	static readCharset(data, offset, len) {
		...
		else if(format==1 || format==2)
		{
			while(charset.length<len)
			{
				...
				for(var i=0; i<= nLeft; i++)  {  charset.add(first);  first++;  }
			}
		}
	...
	static readIndex(data, offset, inds) {
		...
		var count = TyprBin.readUshort(data, offset)+1;  offset+=2;
		var offsize = data[offset];  offset++;
		
		if     (offsize==1) for(var i=0; i<count; i++) inds.add( data[offset+i] );
		else if(offsize==2) for(var i=0; i<count; i++) inds.add( TyprBin.readUshort(data, offset+i*2) );
		else if(offsize==3) for(var i=0; i<count; i++) inds.add( TyprBin.readUint  (data, offset+i*3 - 1) & 0x00ffffff );
    ...
    // var b4=data[offset+4];
		...
		// operand
		if(b0 != null && b0<=20) { op = b0;  vs=1;  }
		if(b0 != null && b0==12) { op = b0*100+b1!;  vs=2;  }
		//if(b0==19 || b0==20) { op = b0/*+" "+b1*/;  vs=2; }
		if(b0 != null && 21 <=b0 && b0<= 27) { op = b0;  vs=1; }
		if(b0==28) { val = TyprBin.readShort(data,offset+1);  vs=3; }
		if(b0 != null && 29 <=b0 && b0<= 31) { op = b0;  vs=1; }
		if(b0 != null && 32 <=b0 && b0<=246) { val = b0-139;  vs=1; }
		if(b0 != null && 247<=b0 && b0<=250) { val = (b0-247)*256+b1!+108;  vs=2; }
		if(b0 != null && 251<=b0 && b0<=254) { val =-(b0-251)*256-b1!-108;  vs=2; }
		if(b0 != null && b0==255) {  val = TyprBin.readInt(data, offset+1)/0xffff;  vs=5;   }
	...
	static readCharString(data, offset, length) {
      ...
      // var b4=data[offset+4];
			...
			if(b0 != null && b0<=20) { op = b0;  vs=1;  }
			if(b0 != null && b0==12) { op = b0*100+b1!;  vs=2;  }
			if(b0 != null && b0==19 || b0==20) { op = b0/*+" "+b1*/;  vs=2; }
			if(b0 != null && 21 <=b0 && b0<= 27) { op = b0;  vs=1; }
			if(b0 != null && b0==28) { val = TyprBin.readShort(data,offset+1);  vs=3; }
			if(b0 != null && 29 <=b0 && b0<= 31) { op = b0;  vs=1; }
			if(b0 != null && 32 <=b0 && b0<=246) { val = b0-139;  vs=1; }
			if(b0 != null && 247<=b0 && b0<=250) { val = (b0-247)*256+b1!+108;  vs=2; }
			if(b0 != null && 251<=b0 && b0<=254) { val =-(b0-251)*256-b1!-108;  vs=2; }
    ...
    int _len = data.length;
      ...
      // var b4 = data[offset+4];
			...
			// operand
			if(b0==28) { val = TyprBin.readShort(data,offset+1);  vs=3; }
			if(b0==29) { val = TyprBin.readInt  (data,offset+1);  vs=5; }
			if(b0 != null && 32 <= b0 && b0 <= 246) { val = b0-139;  vs=1; }
			if(b0 != null && b1 != null && 247<=b0 && b0<=250) { val = (b0-247)*256+b1+108;  vs=2; }
			if(b0 != null && b1 != null && 251<=b0 && b0<=254) { val =-(b0-251)*256-b1-108;  vs=2; }
			if(b0==255) {  val = TyprBin.readInt(data, offset+1)/0xffff;  vs=5;  throw "unknown number";  }
			...
			{  
				...
				while(true)
				{
					...
					if(nib0 != 0xf) nibs.add(nib0);  if(nib1!=0xf) nibs.add(nib1);
					if(nib1==0xf) break;
				}
				var s = "";
