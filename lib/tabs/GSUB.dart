...
class Typr_GSUB {
  ...
  static subt(data, ltype, offset, ltable) // lookup type
  {
    ...
    //  Ligature Substitution Subtable
      ...
      offset += 2;
      for (var i = 0; i < cnt; i++) {
        var loff = TyprBin.readUshort(data, offset);
        offset += 2;
    ...
    //  Contextual Substitution Subtable
      ...
      if (tab["fmt"] == 2) {
        var cDefOffset = TyprBin.readUshort(data, offset);
        offset += 2;
        tab["cDef"] = Typr_LCTF.readClassDef(data, offset0 + cDefOffset);
        tab["scset"] = [];
        var subClassSetCount = TyprBin.readUshort(data, offset);
        offset += 2;
        for (var i = 0; i < subClassSetCount; i++) {
          var scsOff = TyprBin.readUshort(data, offset);
          offset += 2;
          tab["scset"].add(
  ...
  static readLigatureSet(data, offset) {
    var offset0 = offset, lset = [];
    var lcnt = TyprBin.readUshort(data, offset);
    offset += 2;
  ...
  static readLigature(data, offset) {
    ...
    offset += 2;
    var ccnt = TyprBin.readUshort(data, offset);
    offset += 2;
