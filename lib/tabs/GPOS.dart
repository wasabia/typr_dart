...
class Typr_GPOS {
  ...
  static subt(data, ltype, offset, ltable) // lookup type
  {
    ...
    if (ltype == 1 ||
        ...
        ltype == 7 ||
        (ltype == 8 && tab["fmt"] <= 2)) {
      var covOff = TyprBin.readUshort(data, offset);
      offset += 2;
      tab["coverage"] = Typr_LCTF.readCoverage(data, covOff + offset0);
    }
    if (ltype == 1 && tab["fmt"] == 1) {
      var valFmt1 = TyprBin.readUshort(data, offset);
      offset += 2;
      ...
      var valFmt1 = TyprBin.readUshort(data, offset);
      offset += 2;
      var valFmt2 = TyprBin.readUshort(data, offset);
      offset += 2;
      var ones1 = Typr_LCTF.numOfOnes(valFmt1);
      var ones2 = Typr_LCTF.numOfOnes(valFmt2);
      if (tab["fmt"] == 1) {
        tab["pairsets"] = [];
        var psc = TyprBin.readUshort(data, offset);
        offset += 2; // PairSetCount

        for (var i = 0; i < psc; i++) {
          var psoff = offset0 + TyprBin.readUshort(data, offset);
          offset += 2;

          var pvc = TyprBin.readUshort(data, psoff);
          psoff += 2;
          var arr = [];
          for (var j = 0; j < pvc; j++) {
            var gid2 = TyprBin.readUshort(data, psoff);
            psoff += 2;
            var value1, value2;
            if (valFmt1 != 0) {
      ...
      if (tab["fmt"] == 2) {
        var classDef1 = TyprBin.readUshort(data, offset);
        offset += 2;
        var classDef2 = TyprBin.readUshort(data, offset);
        offset += 2;
        var class1Count = TyprBin.readUshort(data, offset);
        offset += 2;
        var class2Count = TyprBin.readUshort(data, offset);
        offset += 2;
      ...
      var extType = TyprBin.readUshort(data, offset);
      offset += 2;
      var extOffset = TyprBin.readUint(data, offset);
      offset += 4;
      if (ltable.ltype == 9) {
