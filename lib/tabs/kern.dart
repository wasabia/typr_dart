...
class Typr_KERN {
  static parse(data, offset, length, font) {
    ...
    var nTables = TyprBin.readUshort(data, offset);
    offset += 2;
  ...
  static parseV1(data, offset, length, font) {
    var version = TyprBin.readFixed(data, offset);
    offset += 4;
    var nTables = TyprBin.readUint(data, offset);
    offset += 4;
    ...
    for (var i = 0; i < nTables; i++) {
      ...
      offset += 4;
      var coverage = TyprBin.readUshort(data, offset);
      offset += 2;
      var tupleIndex = TyprBin.readUshort(data, offset);
      offset += 2;
      var format = coverage >> 8;
      /* I have seen format 128 once, that's why I do */ format &= 0xf;
      if (format == 0)
  ...
  static readFormat0(data, offset, map) {
    var pleft = -1;
    var nPairs = TyprBin.readUshort(data, offset);
    offset += 2;
    ...
    for (var j = 0; j < nPairs; j++) {
      var left = TyprBin.readUshort(data, offset);
      offset += 2;
      var right = TyprBin.readUshort(data, offset);
      offset += 2;
      var value = TyprBin.readShort(data, offset);
      offset += 2;
      if (left != pleft) {
        map.glyph1.add(left);
